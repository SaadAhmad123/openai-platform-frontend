...
function addWidgetToPage(
  ...
  name: string,
  prompt_id: string,
  srcUrl = 'https://open-ai-client.saad-ahmad.com/dist',
) {
...
const InfoPanelForStackV1Pro = ({ stack, IdToken }: InfoPanelForStackV1) => {
  ...
  try {
    ...
    ingestionUrl = stackContent?.find(
      (item: any) =>
        item.ExportName === `${stack?.stack_uuid}-ingest--post-noauth`,
    )?.OutputValue
    deleteIngestedUrl = stackContent?.find(
      (item: any) =>
        item.ExportName === `${stack?.stack_uuid}-ingest--delete-auth`,
    )?.OutputValue
    updateOpenAIKeyUrl = stackContent?.find(
      (item: any) =>
        item.ExportName ===
        `${stack?.stack_uuid}-open-ai-key-update--post-auth`,
    )?.OutputValue
    clientWebSocketUrl = stackContent?.find(
      (item: any) =>
        item.ExportName === `${stack?.stack_uuid}-websocket-noauth`,
    )?.OutputValue
    downloadIngestionUrl = stackContent?.find(
      (item: any) =>
        item.ExportName === `${stack?.stack_uuid}-ingest--get-auth`,
    )?.OutputValue
    promptGetUrl = stackContent?.find(
      (item: any) =>
        item.ExportName === `${stack?.stack_uuid}-prompt--get-auth`,
    )?.OutputValue
    promptPostUrl = stackContent?.find(
      (item: any) =>
        item.ExportName === `${stack?.stack_uuid}-prompt--post-auth`,
    )?.OutputValue
    createIndexGetUrl = stackContent?.find(
      (item: any) =>
        item.ExportName === `${stack?.stack_uuid}-create-index--get-auth`,
    )?.OutputValue
    clientWebAppLink = `https://mgpt-client.saad-ahmad.com/?socket=${clientWebSocketUrl}&name=${
      stack?.name || 'unknown'
    }&prompt_type=default`
  } catch (e) {
    safeConsole()?.error(e)
  ...
  const handleIngestedDataDownload = usePromise(async () => {
    ...
    const downloadResp = await axios.get(resp.data.url, {
      responseType: 'blob',
    })
  ...
  const createIndex = usePromise(async () => {
    return await axios.get(createIndexGetUrl, {
      headers: {
        'Content-Type': 'application/json',
        Authorization: IdToken || '',
      },
  ...
  return (
    <div className="grid gap-3 grid-cols-1 md:grid-cols-2 lg:grid-cols-4">
      ...
      <InfoTile>
        <p>
          Try <strong>{stack?.name} GPT</strong> instantly with our embeddable
          chatbot app.
        </p>
        <Separator padding={8} />
        <HomePageActionButton
          text={`Try ${stack?.name} GPT chatbot!`}
          onClick={() =>
            addWidgetToPage(clientWebSocketUrl, stack?.name || '', 'default')
          }
        />
      </InfoTile>
      <InfoTile>
        <p>
          Try <strong>{stack?.name} GPT</strong> embeddable chatbot in your
          code. Paste it in your code/ console.
        </p>
        <Separator padding={8} />
        <CopyButton
      ...
      <InfoTile>
        <p>
          Manually initiate Index creation. Otherwise, it will be triggered ever
          6 hours.
        </p>
        <Separator padding={8} />
        <HomePageActionButton
      ...
      <InfoTile className="md:col-span-2 lg:col-span-4">
        ...
        <div className="flex items-center gap-4 flex-wrap">
          ...
          <HomePageActionButton
            ...
            text={
              handleIngestedDataDownload?.error?.message || 'Download Index'
            }
            icon={
