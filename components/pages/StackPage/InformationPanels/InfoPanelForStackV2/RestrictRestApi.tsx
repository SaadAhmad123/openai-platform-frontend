...
const RestrictRestApi = ({
    ...
    const configData = usePromise(async () => {
        try {
            ...
            const resp = await axios.post(fetchConfigUrl, undefined, {
                headers: {
            ...
            setIpValues(
                Object.assign(
                    {},
                    ...((resp?.data as any)?.policies || []).map((item: any) => ({
                        [item.api]: ((item.allowedIdAddresses || []) as string[]).join(','),
                    })),
                ),
            )
            return (resp?.data as any)?.expectedRequestBody
        } catch (e) {
            safeConsole()?.error(e)
            return (
                ...
                <>
                    ...
                    <Form
                        ...
                        formValues={{
                            ...
                            allow_ip_list: ipValues[configType || ''],
                        }}
                    />
                    <Separator padding={8} />
                </>
...
export default RestrictRestApi
