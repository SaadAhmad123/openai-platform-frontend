...
const InfoPanelForStackV2 = ({ stack, IdToken }: IInfoPanelForStackV2) => {
    ...
    const openAiWebsocketNoAuth = getStackValue(stack, '-openai--websocket-noauth')
    const clientWebAppLink = `https://mgpt-client.saad-ahmad.com/?socket=${openAiWebsocketNoAuth}&name=${stack?.name || 'unknown'}&prompt_type=default`
    const indexCleanerPostAuth = getStackValue(stack, "-index-cleaner-cron--post-auth")

    const cleanIndex = usePromise(async () => {
        try {
            const resp = await axios.post(indexCleanerPostAuth, {}, {
                headers: {
    ...
    return (
        <div className="grid gap-3 grid-cols-1 md:grid-cols-2 lg:grid-cols-4">
            ...
            <InfoTile>
                ...
                <p>
                    You can manully clean this index
                </p>
                <Separator />
                <Separator />
                <HomePageActionButton
